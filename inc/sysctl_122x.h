/*
 * @brief LPC122X System Control registers and control functions
 *
 * @note
...
#ifndef __SYSCTL_122X_H_
#define __SYSCTL_122X_H_
...
typedef struct {			/*!< SYSCTL Structure */
	...
	__IO uint32_t  RES_LFOSCCTRL;		/*!< LF oscillator control, LPC11AXX only */
	...
	__IO uint32_t  UART0CLKDIV;	/*!< UART clock divider register   */
	__IO uint32_t  UART1CLKDIV;	/*!< SSP1 clock divider register, not on CHIP_LPC110X, CHIP_LPC11XXLV */
	...
	__IO uint32_t  IOCONFIGCLKDIV6;
	__IO uint32_t  IOCONFIGCLKDIV5;
	__IO uint32_t  IOCONFIGCLKDIV4;
	__IO uint32_t  IOCONFIGCLKDIV3;
	...
	__IO uint32_t  STARTRSRP1CLR;	/*!< Start loigc reset register 0, on CHIP_LPC12XX */
	__IO uint32_t  STARTSRP1;		/*!< Start loigc status register 0, on CHIP_LPC12XX */
...
typedef enum {
	...
	RESET_CRC,			/*!< DAC reset control */
	RESET_DMA,			/*!< ADC reset control */
	RESET_FLASH_OVRD = 15
...
#define SYSCTL_WAKEUP_RTC 		(1 << 18)	/*!< USB need_clock signal wake-up, LPC11Uxx only */
...
#define SYSCTL_SLPWAKE_COMP_PD 	(1 << 15)	/*!< Comparator power-down wake-up configuration */
...
#define SYSCTL_POWERDOWN_COMP_PD    (1 << 15) /*!< Comparator power down */
